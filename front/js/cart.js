...
const cartDisplay = async (products) => { 
  ...
  if(products){  
    document.getElementById('cart__items').innerHTML = products.map((product) => `
...
const modifyQuantity = async (products) => {
  ...
  modifyBtnInput.forEach((nvQuantity) => {
    nvQuantity.addEventListener('input', () => {                    // the input of a sinle object to change   
      for (i=0; i < products.length; i++){
        if(products[i]._id == nvQuantity.dataset.id && products[i].color == nvQuantity.dataset.color) {
  ...
  delBtn.forEach((button_delete) => {
    button_delete.addEventListener('click', () => {
      ...
      if( productsRemove == 1) {
        ...
        if(
          button_delete.dataset.id != el._id || button_delete.dataset.color != el.color
          ){
            return true
          }
        })
...
const getTotalPrice = async (products) => {
  ...
  for (i=0; i < products.length; i++){
    const each_product__total_price = eval(products[i].quantity * products[i].price) //totalPrice for a single position 
  ...
  const all_products__total_price__result = eval(each_product__total_price__array.join('+')) 
  
  document.getElementById('totalQuantity').innerHTML = `<p>Total (<span id="totalQuantity">${each_product__quantity}</span> articles) : <span id="totalPrice"></span>${all_products__total_price__result} €</p>`
...
function completeFirstName(e) {
  ...
  if(!e.target.value.match(/^[a-z A-Z]{3,25}$/) && e.target.value.length > 3 &&
  e.target.value.length < 25 ) {
    firstNameErrorMsg.innerHTML ='Prenom contient que des majuscules et minuscules'
    valueFirstName = null
    localStorage.removeItem('firstName')
  }
...
function completeLastName(e){
  ...
  if(!e.target.value.match(/^[a-z A-Z]{3,25}$/) && e.target.value.length > 3 &&
  e.target.value.length < 25 ) {
    lastNameErrorMsg.innerHTML ='Nom ne contient que des lettres majuscules et minuscules'
    valueLastName = null
    localStorage.removeItem('lastName')
  }
...
function completeAddress(e){
  ...
  if(e.target.value.length == 0) {
    ...
    addressErrorMsg.innerHTML = 'Adresse doit contenir entre 3 et 35 caractéres'
    valueAddress = null
  ...
  if(!e.target.value.match(/^[0-9]{1,3} [a-z A-Z]{3,35}$/) && e.target.value.length > 3 &&
  e.target.value.length < 35 ) {
    addressErrorMsg.innerHTML ='Adresse commence par des chiffres suivis par des lettres'
    valueAddress = null
    localStorage.removeItem('address')
  }
  return valueAddress
...
function completeCity(e){
  ...
  if(!e.target.value.match(/^[a-z A-Z]{3,25}$/) && e.target.value.length > 3 &&
  e.target.value.length < 25 ) {
    cityErrorMsg.innerHTML ='Ville ne contient que des lettres majuscules et minuscules'
    valueCity = null
    localStorage.removeItem('city')
  }
  return valueCity
...
async function prepareOrder () {
  ...
  const orderFinal = JSON.parse(localStorage.getItem('product'))
  const orderId = []
  const orderMoreInfo = []    
  //Push all id-products into the array orderId
  orderFinal.forEach((each_product_cart) => {
    orderId.push(each_product_cart._id)
    //The object  data_each_product_cart need to push into the array "orderMoreInfo". OrderMoreInfo is prepared to push into the "data"
    const data_each_product_cart = {       
      id : each_product_cart._id,                   
      color: each_product_cart.color,              
      quantity: each_product_cart.quantity          
    }
    orderMoreInfo.push(data_each_product_cart)
  }) 
  const data = {
    contact:{
    ...
    products: orderId,
    productsMoreInfo : orderMoreInfo
  }
  console.log(data)
  return data
} 
//----------------- fetch post---------------------------
const postDataOrder = (order) => fetch('http://localhost:3000/api/products/order',{
    method: 'POST',
...
//!!!!!!!!without ProductsMoreInfo!!!!!!!!!!!!! 

async function optimisationDataOrder (order, price) {
  //await postDataOrder 
  //console.log(responseServer)                             //without productsMoreInfo
  const dataOrder = {                                     //responseServer after await postDataOrder in the SendOrder                       
    contact: responseServer.contact,                      // from responseServer              
    order: responseServer.orderId,                        // from responseServer                    
    products: responseServer.products,                    // from responseServer                    
    
    productsMoreInfo: order.productsMoreInfo,             //from order
...
const main = async () => {
  ...
  sendForm.addEventListener('submit', async function sendOrder(e){
    e.preventDefault(price) //to stop sending the form
    ...
    if(valueFirstName && valueLastName && valueAddress && valueCity && valueEmail){
      ...
      optimisationDataOrder (order,price)
    }
  })
}  
main()
